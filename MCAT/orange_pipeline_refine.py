...
NUM_OF_TOOLS = 6
MOTIF_LEN = 8
NUM_MOTIFS = 5 #number of motifs expected per sequence
DECOD_ITER = 25 #number of iterations to perform
...
def smooth(poll):
    for seq in poll:
        for i in xrange(1, len(poll[seq]) - 1):
             if poll[seq][i - 1] == poll[seq][i + 1] and poll[seq][i] < poll[seq][i - 1]:
                 poll[seq][i] = poll[seq][i - 1]


        for i in xrange(1, len(poll[seq]) - 2):
             if poll[seq][i - 1] == poll[seq][i + 2] and poll[seq][i] < poll[seq][i - 1]:
                 poll[seq][i] = poll[seq][i - 1]
             if poll[seq][i - 1] == poll[seq][i + 2] and poll[seq][i + 1] < poll[seq][i - 1]:
                 poll[seq][i + 1] = poll[seq][i - 1]

def compMotifPWM(motif, PWM):
     matching = 0
     insts = sum(PWM[0])
     for c, col in zip(motif, PWM):
         matching += col[ALPH[c]]/(.75*insts)
         #if col[ALPH[c]] == max(col):
         #    matching += 1
     return matching

def logLikelihood(PWM, probs):
     ll = 0
     for col in PWM: 
        ll += max(col) / (sum(col) * probs[ALPHl[col.index(max(col))]])
     return log(ll, 2)

def addMotif(PWM, motif):
    newPWM = json.loads(json.dumps(PWM))
    for c in motif:
        newPWM[alph[c]] += 1
    return newPWM

def PWMconsensus(PWM):
     s = ''
     for col in PWM:
         s += ALPHl[col.index(max(col))]
     return s
...
def voteRankMotifs():
    ...
    results = voteRefine(seqs, foundMotifsSeqs)
    print('results.....')
    


#    global mlist
    results = [list(motif_tuple[::-1]) for motif_tuple in results[::-1]]
    print results

    #mlist.append([x[0] for x in results[::-1]])
...
def scoreMotifs():
    ...
    for mnum, (motif, pval) in enumerate(pvals_comb_qfast):
...
#    config['WEEDER']['-O'] = 'SP'
    ...
    config['XXmotif']['-g'] = '0'
    config['XXmotif']['--background-model-order'] ='8'
...
def parseArgs():
    ...
    opt.add_argument('--nmotifs', default=5, type=int, help='max number of distinct motifs to look for')
    opt.add_argument('--iter', default=25, type=int, help='max number of iterations (for DECOD tool)')
    ...
    opt.add_argument('-v', default=5, type=int, help='number of results to include in visual output')
    args=opt.parse_args()
...
def main():
    ...
    # resort motifs based on p-value
    mlist.sort(key=lambda mlist:mlist[2])
    print mlist
    # find all instances of found motifs
    for mnum, m in enumerate(mlist):
        searchFasta(FILTERED_SEQ, RES_DIR, m[1], mnum+1)
