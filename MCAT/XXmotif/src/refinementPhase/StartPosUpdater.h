...
public:
  ...
  //void update_pwm_with_bestSubset_All(Motif* motif, double pseudo);
  //void update_pwm_with_bestSubset_PlusAll(Motif* motif, double pseudo);
  //void update_pwm_with_bestSubset_Plus(Motif* motif, double pseudo);
...
//	const int totalMotifLength = columns.back() - columns.front() + 1;
	double log_prod_pcons = 0;
	double log_ranks_pValue = 0;
	double log_prod_pdiso = 0;
	...
	for(int i=0; i<std::min(nbSites, posSetSize*Global::maxSeqCount); i++){
		...
		if(Global::posSet->max_MultSeq > 1){
			...
			log_prod_pcons += log(pValCons);
			//if(Global::useAliFree) log_prod_pcons_free += log(pValAliFree);
		}
		...
		if (Global::supplInf != NULL) {
			const double p_diso = Global::supplInf->getPvalue(&columns, sites[i]->sequence,
					sites[i]->startPos);
			log_prod_pdiso += log(p_diso);
			const double pValDiso = pValCalculator.getCombinedPval_log(
					log_prod_pdiso, i + 1);
//			fprintf(stderr, "p_diso=% .6f   pValDiso[%1.6e,%2d]==% .6f      ", p_diso, log_prod_pdiso, i+1, pValDiso);
			pValue = pValCalculator.getWeightedPval_log(pValue, pValDiso,
				Global::disoconsWeight);
		}
...
//	if(Global::posSet->max_MultSeq > 1){
//		/* order statistics for conservation */
//		qsort(pvalCons, bestMotifNb, sizeof(double), sort);
//		double bestPvalCons = 1;
//		for(int i=0; i<bestMotifNb; i++){
//			double pValueCons = calculateOrderStatisticsPvalue(i+1, bestMotifNb, pvalCons[i]);
//			if(pValueCons < bestPvalCons){
//				bestPvalCons = pValueCons;
//			}
//		}
//
//		if(debug)fprintf(stderr, "\toverRepPval: %f (%e), sites: %d", bestPval, exp(bestPval), bestMotifNb);
//		bestPval = pValCalculator.getWeightedPval(bestPval, bestPvalCons, Global::consPvalWeight);
//		if(debug)fprintf(stderr, "\tpValCons: %f (%e) \n\tpValCombined = %f (%e)\n\n", bestPvalCons, exp(bestPvalCons), bestPval, exp(bestPval));
//	}
