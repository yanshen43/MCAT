	...
	if(Global::revcomp && pos > length / 2){
		...
		if(sites_counter - back >= 0 &&
		   ...
		   sites[sites_counter - back]->startPos >= revPos -length +1){
			...
			//if(pVal < 1e-3){
			//	fprintf(stderr, "%d/%d/%f vs %d/%d/%f => %d vs %d\n", sites[sites_counter - back]->sequence,
			//		sites[sites_counter - back]->startPos,
			//		sites[sites_counter - back]->score, seq, pos, pVal,
			//		sites[sites_counter - back]->startPos, revPos);
			//}
