...
inline float log2(float x) {
  return static_cast<float>(1.442695041f * log(x));
}
...
inline double rel_diff(double logp, double logp_prev) {
  return (logp - logp_prev) / std::max(fabs(logp), fabs(logp_prev));
}
...
inline unsigned int random_seed() {
 unsigned int seed;
 struct timeval tv;
 FILE *devrandom;
...
inline int chomp(char* str) {
  if (!str) return -1;
  int l = 0;
  for (l = static_cast<int>(strlen(str)) - 1; l >= 0 && str[l] < 32; --l)
    /* do nothing */;
  str[++l] = '\0';
  return l;
}
