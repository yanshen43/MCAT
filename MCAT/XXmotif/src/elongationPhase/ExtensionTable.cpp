...
void ExtensionTable::initialize(MProGlobal::SeqInfo_t &_S, const double p_thresh, const double seqThresh,
	...
	for (int i = 0; i < static_cast<int>(S->alphabet.size()); ++i) {
		...
		for (int s = 0; s < stateSize; ++s) {
			if (Global::aa) { /* copy significant states already calculated in StateLib */
				for (StateLib::pair_list_type::const_iterator sig_it = S->states.sigStates[i].begin(); sig_it
						!= S->states.sigStates[i].end(); ++sig_it) {
					sigStates[i].push_back(sig_it->second);
				}
			} else {
