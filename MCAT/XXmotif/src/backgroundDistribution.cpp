...
void setBackgroundDistribution(){
	...
	if( Global::em ){
		if( Global::negSet != NULL ){
			hoNullModel::init( Global::negSet, Global::alphaBg, Global::betaBg, Global::modelOrderBg, Global::gaps, Global::freqs );
		} else{
			hoNullModel::init( Global::posSet, Global::alphaBg, Global::betaBg, Global::modelOrderBg, Global::gaps, Global::freqs );
		}
	}
}
...
void checkSequenceSet(){
	...
	if(Global::negSet != NULL){
		for(int i=1; i <= Global::negSet->nent; i++){
			...
			//for(k=0; k< Global::negSet->entity[i]->mseq; k++){
			for(int k=0; k<1; k++){
				totNeg += len;
	...
	for(int i=1; i <= Global::posSet->nent; i++){
		...
		//for(k=0; k<Global::posSet->entity[i]->mseq; k++){
		for(int k=0; k<1; k++){
			totPos += len;
	...
	if (Global::aa) {
		if (Global::negSet != NULL) {
			double neg_dollars = Global::negBg[AlphaCode('$',Global::A)];
			totNeg += (Global::dollarScaleFactor - 1) * neg_dollars;
			Global::negBg[AlphaCode('$',Global::A)] = Global::dollarScaleFactor * neg_dollars;
		}
		double pos_dollars = Global::posBg[AlphaCode('$',Global::A)];
		totPos += (Global::dollarScaleFactor - 1) * pos_dollars;
		Global::posBg[AlphaCode('$',Global::A)] = Global::dollarScaleFactor * pos_dollars;
	}
	for(int i=1;i<aS;i++){
		if(Global::negSet != NULL){
