...
//double calculate_log_bonferonni(const int length, const int matchPositions, const double log_neff, motif_type type);
...
inline double calculate_log_bonferonni(const motif_columns_type& motif_columns, const double log_neff){
	if(Global::aa){
		int usedLength = motif_columns.back() - motif_columns.front() + 1;
		int usedMatchPositions = (int)motif_columns.size();
		if (Global::fixedPosition){
			// fixed $ column does not count
			--usedMatchPositions;
			--usedLength;
		}

		return usedMatchPositions*log_neff + log_binomial_coefficient(usedLength - 1, usedMatchPositions-1, LogTable::LOG_sum_i);
	}else{
...
//		//fprintf(stderr, "%d * %f = %f (%e)\n", (int)motif_columns.size(), log_neff, motif_columns.size()*log_neff, exp(motif_columns.size()*log_neff));
	//	return motif_columns.size()*log_neff + gapPenalty*LogTable::LOG_i[2];
		return (motif_columns.back() - motif_columns.front() + 1)*log_neff;
	}
}

///* calculate log( neff^K * (N-1 over K-1) ) , N: length, K: matchPositions*/
//inline double calculate_log_bonferonni(const int length, const int matchPositions, const double log_neff, motif_type type){
//	int usedLength = length;
//	int usedMatchPositions = matchPositions;
//	/* for palindromes calculate bonferonni correction just for one half */
//	//if(type == PALINDROME){
//	//		usedLength /= 2;
//	//		usedMatchPositions /= 2;
//	//}
//
//	double LOG_Bonferonni = usedMatchPositions*log_neff + log_binomial_coefficient(usedLength - 1, usedMatchPositions-1, LogTable::LOG_sum_i);
//
//	/* if palindrome has a middle position, correct it with only half of neff */
//	//if(type == PALINDROME && length % 2 == 1) LOG_Bonferonni += LogTable::LOG_i[Global::neff_pwm / 2 ];
//
//	return LOG_Bonferonni;
//}
...
inline double calculateDistanceFromRandom(const int motifNb, const int N, const double p){
	//double cutPoint = (motifNb*1.0/N + p)/2;
	//double dist = sqrt(2*pow(motifNb*1.0/N - cutPoint, 2));
	//return dist;
