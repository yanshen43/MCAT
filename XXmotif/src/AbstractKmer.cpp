...
std::string AbstractKmer::matchedCharsStartingAt(const Sequence &seq,
    const int start) const {
  const Alphabet &alphabet = MProGlobal::getS().alphabet;
  int seqPos = start;
  std::string s;
  for (int matchPos = 0; matchPos < numMatches() - 1; ++matchPos) {
    s += alphabet.itoc(seq[seqPos]);
    seqPos += 1 + gapsAfter(matchPos);
  }
  s += alphabet.itoc(seq[seqPos]);
  return s;
}
