...
GETOPT_INLINE std::list<std::string> GetOpt_pp::remaining_options() const
{
	std::list<std::string> unp;
	for (ShortOptions::const_iterator it = _shortOps.begin(); it != _shortOps.end(); ++it)
	{
		if (it->second.flags == OptionData::CmdLine_NotExtracted) {
			unp.push_back(std::string(1, it->first));
		}
	}

	for (LongOptions::const_iterator it = _longOps.begin(); it != _longOps.end(); ++it)
	{
		if (it->second.flags == OptionData::CmdLine_NotExtracted) {
			unp.push_back(it->first);
		}
	}
