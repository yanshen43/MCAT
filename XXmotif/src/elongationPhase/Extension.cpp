...
#include "../aminoacids/IntervalSet.h"
...
void Extension::initialize() {
	if (Global::aa) {
		S = MProGlobal::getS();
		ExtensionTable::initialize(S, Global::aaStateSigThresh, Global::aaSeqFreqThresh);
	} else {
		S.D_MAX = 4;
	...
	for (elongList::const_iterator kres = krs.begin(); kres != krs.end(); ++kres) {
		...
		if (vizopts && vizopts->stream) {
			*(vizopts->stream) << '\t' << '"'
			...
			if (Global::aa) {
				*(vizopts->stream) << (*kres)->getKmer()->bestAAString()
						<< ": " << (*kres)->getKmer()->toString(0) << "\\n";
			} else {
		...
		for(std::list<int>::iterator it = interestingPositions.begin(); it != interestingPositions.end(); it++){
			...
			if (debugOut){
				...
				if(Global::aa) cout	<< (*kres)->getKmer()->toString(0) << endl << std::flush;
				else cout	<< (*kres)->getKmer()->toString(0, ElongCandidates::IUPAC_CHARS) << endl << std::flush;
			}
			...
			if(!hashMotif){
				...
				while (mit != (*kres)->seeds.end()) {
					...
					}else if (Global::aa && S._posSet[mit->seq][mit->pos+offset] == S.alphabet.wildcardIndex()) {
						// match is 'X' masked, eliminated later
					}else {
						ExtensionTable::count(mit->seq, S._posSet[mit->seq][mit->pos + offset], Global::multipleOccurrence&&!Global::aa);
						++num_seeds;
					}
					++mit;
				}
...
//					}
			...
			if (!estates.empty() && (*kres)->getKmer()->numMatches() < Global::maxMatchPositions) {
				...
				for (ExtensionTable::stateList_type::const_iterator
					...
					if (debugOut){
						if(Global::aa) cout << "\t" << cand->getKmer()->toString(0) << ": " << std::flush;
						else cout << "\t" << cand->getKmer()->toString(0, ElongCandidates::IUPAC_CHARS) << ": " << std::flush;
					}
							...
							if(calibratedMotifs.find(id) == calibratedMotifs.end()){
								...
								if (pVal <= (*kres)->p_set){
									...
									*(vizopts->stream) << '\t' << '"'
									...
									if (Global::aa) {
										*(vizopts->stream)
										<< cand->getKmer()->bestAAString()
										<< ": "
										<< cand->getKmer()->toString(0);
									} else {
					...
					if(hashHit){
						...
						while (mit != (*kres)->seeds.end()) {
							...
							if (newStart >= firstCharSeq && newEnd < (int)S._posSet[mit->seq].size()) {
								...
								if(Global::aa){
									if (base != S.alphabet.wildcardIndex()) {
										cand->seeds.push_back(Match(mit->seq, newStart));
									}
								}else{
									switch(*state_it){
						...
						if (Global::aa)	MadonaPro::calibrateMotif(*cand);
						else calibrateMotif(*cand, debugOut);
						//if (debugOut) cerr << "New candidate (calibrated):  " << *cand << endl;

						calibratedCount++;
//						if ((cand->p_set <= (*kres)->p_set && cand->setSize >= Global::minCoverage) ||
//							(!Global::aa && *state_it < 4 && cand->setSize == (*kres)->setSize) ) {
								...
								*(vizopts->stream) << '\t' << '"'
								...
								if (Global::aa) {
									*(vizopts->stream)
									...
									//*(vizopts->stream) << cand->getKmer()->bestNucString()
									*(vizopts->stream) << cand->getKmer()->toString(0, ElongCandidates::IUPAC_CHARS);
								}
								*(vizopts->stream) << "\\n" << exp(cand->p_set);
								if(!hashMotif) *(vizopts->stream) << "\\n" << cand->setSize;
		...
		while (cand_it != nextCandidates.end()) {
			if (vizopts && vizopts->stream) {
				*(vizopts->stream) << '\t' << '"'
				...
				if (Global::aa) {
					*(vizopts->stream)
							<< (*cand_it)->getKmer()->bestAAString()
							<< ": "
							<< (*cand_it)->getKmer()->toString(0);
				} else {
