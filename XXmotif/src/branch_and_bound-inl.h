...
Kmer_Generator_Reloaded<HASH>::Kmer_Generator_Reloaded() : bgLog((Global::negSet==NULL) ? Global::posBg_log : Global::negBg_log) {
	ID_SHIFT = Global::aa ? 5 : 2;
	CHR_MASK = Global::aa ? 0x1F : 3;
...
inline double Kmer_Generator_Reloaded<HASH>::calculate_kmer_probability(const id_type kmer_id){
	...
	static unsigned char unexpanded_kmer[100];
	id_type id = kmer_id;
	int totalPos = 0;
	for (int matchPos=0; matchPos<length; ++matchPos) {
		/* indices in id are shifted by -1 w.r.t. the alphabet */
		unexpanded_kmer[matchPos] = expanded_kmer[totalPos++] = static_cast<uint8_t>((id & CHR_MASK) + 1);
		id >>= ID_SHIFT;
	...
	double prob;
	if (part==0) {
		//for(int i=0; i<length; i++){fprintf(stderr, "%c", AlphaChar(expanded_kmer[i], Global::A));}
	...
	/* if sequence position is used, return the max of conventional
	 * probability and composition dependent probability
	 */
	if (Global::useCompositionProbs) {
		double prob_comp;
		if (part==0) {
			prob_comp = NullModelCompositional::getProbability(unexpanded_kmer, length);
		} else {
			prob_comp = 1;
			for (int stop=0; stop < length; ++stop) {
				const int start = std::max(0, stop - NullModel::getOrder());
				const int len = stop - start + 1;
				const UngappedKmer k(unexpanded_kmer + start, len);
				if (len <= NullModel::getOrder()) {
					prob_comp *= getRightKmerConditionalCompositional(k);
				} else {
					prob_comp *= NullModelCompositional::getConditional(k);
				}
			}
		}
		prob = std::max(prob, prob_comp);
	}
...
void Kmer_Generator_Reloaded<HASH>::reinitialize(const AbstractKmer &kmer,
	...
	/* split kmer in several parts */
		...
		T = (int)(threshold_left*1e6 - 1);
		prefixList.clear();
		//pValueHash[0].rehash(pow(4,splitAfter+2));
		part = 0;
		...
		//pValueHash[1].rehash(pow(4,totalLength-splitAfter+2));
		fillProbabilityHash = false;
		part = 1;
		...
		//int stop = MIN(int, 2*MAX_KMER_SIZE, totalLength);
...
void Kmer_Generator_Reloaded<HASH>::reinitialize(double const* const* pwm, const motif_columns_type& motif_columns, int pos_set_size, int total_positions) {
	...
	MAX_KMER_SIZE = std::min(totalLength, Global::aa ? 5 : 8);
	if(totalLength > 17){
	...
	if(totalLength <= MAX_KMER_SIZE){
		...
		fillProbabilityHash = false;
		gapOffset = 0;
//		T = totalLength * 1000000; /* TODO B&B threshold */
		T = 0;
		set_kmer_to_Pvalue_hash(1, totalLength);
	/* split kmer in several parts */
	}else{
...
//		T = MAX_KMER_SIZE * 1000000;
		...
		//const int stop = MIN(int, 2*MAX_KMER_SIZE, totalLength);
...
//		condProb += it->probability * NullModel::getConditional(it->kmer.getLastPosCombined(stop-start+1,k));
...
template<class HASH>
float Kmer_Generator_Reloaded<HASH>::getRightKmerConditionalCompositional(const UngappedKmer& k){
	if (rightKmerCondComp.find(k) != rightKmerCondComp.end()) {
		return rightKmerCondComp[k];
	}
	double condProb = 0;
	const int start = MAX_KMER_SIZE - NullModel::getOrder() + k.length() -1;
	const int stop = MAX_KMER_SIZE - 1;
	for (prefix_list_t::iterator it = prefixList.begin(); it != prefixList.end(); ++it) {
		condProb += it->probability * NullModelCompositional::getConditional(it->kmer.getLastPosCombined(stop-start+1,k));
	}
	rightKmerCondComp[k] = static_cast<float>(condProb);
	return rightKmerCondComp[k];
}
...
// 		/*FIXED BUG*/
// 			pValue += static_cast<int>(id_vector.size())*probabilityHash[id_vector.front()] * pVal_s_s1_max;
...
void Kmer_Generator_Reloaded<HASH>::create_similar_kmers_rec(int j, double S_j, unsigned char* idx){
	...
	for(int a=1;a<=nAlpha(Global::A);a++){
		...
		// if(S_j_1 + S_max[j+1] > 0) fprintf(stderr, "j: %d, S_j_1: %.10f, %.10f, sum: %.10f, T: %d\n", j, S_j_1, S_max[j+1], S_j_1 + S_max[j+1], T);
...
//			if(scoreMapArray[intScore].empty()){
//			  scoreMapArray[intScore].reserve(100); // reserve space to prevent reallocation
//			}
//			fprintf(stderr, "generated id %lu, score %d\n", id, intScore);
