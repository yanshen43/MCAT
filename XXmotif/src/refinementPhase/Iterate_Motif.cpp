...
IterateMotif::IterateMotif(){
	...
	skipRoundThreshold = Global::aa ? log(0.5) : log(0.7);
}
...
void IterateMotif::optimizeMotif(Motif* motif, bool keepPosSetSize, bool plus){
	...
	for(int iter = 0; iter < 2; iter++){
		...
		//double pseudo = (iter == 0) ? 1 : Global::pseudo;
		//cout << *motif;
...
bool IterateMotif::maximizeMotifLength(Motif* motif, bool lengthCorrection, bool palin){
	...
	if(Global::aa){
		frontStart = firstMotifColumn-3;
		frontEnd = lengthCorrection ? motif->getLastMotifColumn()+2 : frontStart;
		backStart = 0; backEnd=0;
	}
	...
	for(int front = frontStart; front <= frontEnd; front ++){
		for(int back = backStart; back <= backEnd; back++){
			...
			bool skipRound;
			if(Global::aa){
				skipRound = fillTmpColumn(front == firstMotifColumn-3 ? 1000 : front, tmpColumns, motif);
			}else{
...
//bool IterateMotif::checkPalindrome(Motif* motif, double pseudo){
//	int asize = nAlpha(Global::A);
//	int firstMotifColumn = motif->getFirstMotifColumn();
//	int length = motif->getMotifLength();
//	/* generate palindromic PWM */
//	for(int i = 0; i < length; i++){
//		for(int j=1; j<=asize; j++){
//			elongPWM[i+firstMotifColumn][j] = motif->getPWM()[firstMotifColumn+length-1-i][asize-j+1];
//		}
//	}
//
//	double normalPval = 1;
//	int motifNb = 0;
//
//	//fprintf(stderr, "==== no palin : \n");
//	//printPWM(motif->getPWM(), motif->getMotifColumns(), std::cerr);
//
//	Kmer_Generator_Reloaded<dna_hash_t> &kmerHash = Kmer_Generator_Reloaded<dna_hash_t>::getInstance();
//	kmerHash.reinitialize(motif->getPWM(), motif->getMotifColumns(), motif->getPosSetSize(), Global::posSet->total[motif->getPosSetSize()]);
//	StartPosUpdater::getInstance().fill_elongSites_with_possible_startPositions(kmerHash, motif, motif->getPWM(), motif->getMotifColumns(), false);
//	StartPosUpdater::getInstance().update_elongPWM_with_bestSubset(motif, motif->getPWM(), motif->getMotifColumns(), motifNb, normalPval);
//
//	//printPWM(motif->getPWM(), motif->getMotifColumns(), std::cerr);
//	//fprintf(stderr, "==== palin : \n");
//	//printPWM(elongPWM, motif->getMotifColumns(), std::cerr);
//
//	double palinPval = 1;
//	kmerHash.reinitialize(elongPWM, motif->getMotifColumns(), motif->getPosSetSize(), Global::posSet->total[motif->getPosSetSize()]);
//	StartPosUpdater::getInstance().fill_elongSites_with_possible_startPositions(kmerHash, motif, elongPWM, motif->getMotifColumns(), true);
//	StartPosUpdater::getInstance().update_elongPWM_with_bestSubset(motif, elongPWM, motif->getMotifColumns(), motifNb, palinPval);
//
//	//printPWM(elongPWM, motif->getMotifColumns(), std::cerr);
//	//fprintf(stderr, "normalPval: %e, palinPval: %e\n", normalPval, palinPval);
//
//	//exit(-1);
//	if(normalPval < palinPval){
//		return false;
//	}
//
//	for(int i = 1; i <= PWM_LENGTH; i++){
//		for(int j=1; j<=asize; j++){
//			motif->getPWM()[i][j] = elongPWM[i][j];
//		}
//	}
//	return true;
//}
