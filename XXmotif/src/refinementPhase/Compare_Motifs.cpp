...
int MotifComparison::calcBestMergingDist(Motif* model1, Motif* model2, int &bestRevcomp){
	...
	int revComp = 0;
	if(!Global::aa && Global::revcomp){ revComp = 1; }
...
//	if(sum_identicalMotifs < identical_minMotif_threshold * std::min(model1->getTotalSites(), model2->getTotalSites())
//	  && sum_identicalMotifs < identical_maxMotif_threshold * std::max(model1->getTotalSites(), model2->getTotalSites())
//	){
