...
inline void IterateMotif::fillElongationList(list<int>& elongColumns, Motif* motif, bool lengthCorrection, int offset){
	...
	if(lengthCorrection){
		...
		/* no longer needed with new null model
		 */
//		if (Global::aa && motif->getMotifColumns().size()>2) {
//			/* Removal of inner matchColumns may lead to a gap >D_MAX.
//			 * Therefore, remove these columns from elongColumns.
//			 * Example for testing: ELME000011 with seed IXXXXXY
//			 */
//			const motif_columns_type &mcols = motif->getMotifColumns();
//			list<int>::iterator elong_it = elongColumns.end();
//			--elong_it;
//			int nextGap = 0;
//			motif_columns_type::const_reverse_iterator currCol = mcols.rbegin();
//			motif_columns_type::const_reverse_iterator nextCol = currCol;
//			const int maxGapLength = MProGlobal::getS().D_MAX;
//			while (currCol != mcols.rend()) {
//				const int currGap = nextGap;
//				++nextCol;
//				if (nextCol==mcols.rend()) {
//					nextGap = 0;
//				} else {
//					nextGap = *currCol - *nextCol - 1;
//				}
//				if (nextGap+currGap+1 > maxGapLength) {
//					while (*elong_it != *currCol) --elong_it;
//					elong_it = elongColumns.erase(elong_it);
//				}
//				++currCol;
//			}
//		}
...
inline bool IterateMotif::fillTmpColumn(int mutation, motif_columns_type& tmpColumns, Motif* motif){
	...
	/* current column is before first match column */
		...
		//for(int i= firstMotifColumn-1; i>=mutation; i--){
		//	tmpColumns.push_front(i);
		//}
	}else{
		...
		if(it_column == tmpColumns.end() && mutation != motif->getLastMotifColumn()){
			...
			//for(int i= motif->getLastMotifColumn() + 1; i<=mutation; i++){
			//	tmpColumns.push_back(i);
			//}
		}
	}
