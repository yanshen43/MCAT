...
use lib qw(/home/thejar/lib/perl);
...
unless ($@) {
  Log::Log4perl::init('/home/thejar/etc/logging.conf');
...
sub update {
  ...
  my $template = HTML::Template->new(filename => '/home/thejar/etc/job_status.tmpl');
  $template->param(
...
sub write_log {
  ...
  my $logfile = catfile('/home/thejar/LOGS', $self->{log});
