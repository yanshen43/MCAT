...
our @ISA = qw(Exporter);
our @EXPORT = qw();
our @EXPORT_OK = qw(arg_checks arg_end opt_uploaded opt_db opt_db_or_uploaded opt_choice opt_encoded opt_integer opt_number opt_evalue opt_safe);
...
unless ($@) {
  Log::Log4perl::init('/home/ugrads/nonmajors/jrm98/meme/etc/logging.conf');
...
sub update {
  ...
  my $template = HTML::Template->new(filename => '/home/ugrads/nonmajors/jrm98/meme/etc/job_status.tmpl');
  $template->param(
...
sub run {
  ...
  $self->add_message('Starting '.$prog.'<br><code>' . stringify_args($prog, @args) . '</code>');
...
sub write_log {
  ...
  my $logfile = catfile('/home/ugrads/nonmajors/jrm98/meme/LOGS', $self->{log});
